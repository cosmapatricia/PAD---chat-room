...
int main(int argc , char *argv[])
{
    ...
    if(pid_receive == 0)
    {
        ...
        if(recv(sock, &green_light, sizeof(green_light), 0) > 0  && green_light==1)
        {
            //printf("green_light: %d\n", green_light);
        }
        else
        {            
            printf("Error receiving the green light to send login info\n");
            exit(10);
        }
        
        
        while(recv(sock, &size, sizeof(size), 0) > 0  && recv(sock, server_reply, size, 0) > 0)
        {
