...
{	   readUsersFromFile();
	   printUsers();
		...
		new_sock = malloc(4);   //malloc(4)? new_sock e pointer la int 
		*new_sock = new_socket;
...
void *connection_handler(void *socket_desc)
{
    ...
    {
        //printf("login size: %d\n", size);
        send(sock, &green_light, sizeof(green_light) , 0);
    }
    else
    {
        printf("Error receiving login size\n");
       // exit(7);
    }
	...
	newClient.socket_fd=sock;
        ...
        //client is signing up for the first time -> NEW or is just logining in -> VALID
        if(status==VALID || status==NEW)
	{
		    ...
		    else if(status==VALID)        
		    {
		    		int pos=getClientIndex(newClient);  
                                if(pos==-1)	//the client which exists in the database has not been added to the current session
                                {
                                    clients[n++]=newClient;
                                }
				else
		   ...
		   printf("no of clients for current session: %d\n",n);
 		   ...
 		   strcat(message," has connected.\n");
                   size=strlen(message)+1;       // +1 pt terminatorul de sir           
			
		   for(i=0; i<n; i++)
		   {
                       send(clients[i].socket_fd, &size, sizeof(size), 0);
                       send(clients[i].socket_fd, message, size, 0);
        	   }
	}
	//wrong password case
	else           
	{
		...
		strcpy(msg,"Wrong password!");
                size=strlen(msg)+1;
                send(newClient.socket_fd, &size, sizeof(size), 0);
		send(newClient.socket_fd, msg, size, 0);
		close(sock);
	}		
     	pthread_mutex_unlock(&mutex);
    }
    ...
    //Receive the size, then the message from a client
    while( recv(sock, &size, sizeof(size), 0) > 0  && (read_size = recv(sock, message, size, 0)) > 0 )
    {   
